...
async function findCGVSchedule() {
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (data) {
        console.log('✅ 응답 받음:');
        console.log(typeof data === 'string' ? data.substring(0, 500) : JSON.stringify(data, null, 2).substring(0, 500));
      }
    } catch(error) {
  ...
  console.log('\n\n📝 CGV 사이트 구조 분석 필요');
  console.log('CGV는 최근 사이트를 리뉴얼했으며, 새로운 API 구조를 사용합니다.');
  console.log('브라우저 개발자 도구에서 Network 탭을 열고 예매 페이지를 탐색하면');
