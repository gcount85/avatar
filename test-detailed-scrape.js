...
async function findAvatarShowtimes() {
  ...
  try {
    ...
    page.on('response', async (response) => {
      ...
      if (url.includes('api') || url.includes('schedule') || url.includes('showtime')) {
        console.log('📡 API 호출 감지:', url);
        try {
          const json = await response.json();
          console.log('📄 응답 데이터:', JSON.stringify(json, null, 2).substring(0, 500));
        } catch(e) {
          // JSON이 아닌 경우 무시
        }
      }
    ...
    // 아바타 영화 카드 찾기
    const avatarMovie = await page.locator('text=/아바타.*불과.*재|Avatar.*Fire.*Ash/i').first();
    ...
    console.log(`극장 정보 확인: 용산=${hasYongsan}, 아이파크=${hasIPark}, IMAX=${hasIMAX}`);

    // 용산 아이파크몰 찾기
    try {
      const yongsanTheater = await page.locator('text=/용산.*아이파크|아이파크.*용산/i').first();
      if (await yongsanTheater.isVisible({ timeout: 5000 }).catch(() => false)) {
    ...
    // 날짜 정보 추출
    console.log('\n📅 날짜 정보 찾기:');
    const dateElements = await page.locator('text=/\\d+일|\\d{2}\\/\\d{2}|\\d{4}-\\d{2}-\\d{2}/').all();
    const dates = [];
    for (let i = 0; i < Math.min(dateElements.length, 20); i++) {
      const text = await dateElements[i].textContent();
      if (text.trim().length < 20) {  // 너무 긴 텍스트 제외
    ...
    console.log(`발견된 시간: ${[...new Set(showtimes)].slice(0, 15).join(', ')}`);
